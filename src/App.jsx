...
const MathsInterface = {
  ...
  // Generate hash of design structure (to detect when rebuild is needed)
  _getDesignHash(design) {
    ...
    return `${design.numSequences}-${design.numPeriods}-${structure}-${clusters}`;
  },
  ...
  _buildFormula(design, options) {
    ...
    // Include intercept
    if (options.includeIntercept === false) {
      formula += '-1';
    }
    ...
    switch (options.correlationStructure) {
      ...
      case 'exponential_function':
        formula += `+(${reString}|gr(cl)*sqexp0(t))`;
        if (options.heterogeneousTe) formula += '+(control|gr(cl)*sqexp0(t))';
        break;
      default:
        formula += `+(${reString}|gr(cl))`;
    }
    ...
    if (options.samplingStructure === 'closed_cohort') {
      switch (options.individualCovariance) {
        case 'exchangeable':
          formula += '+(1|gr(cl))';
          break;
        case 'autoregressive':
          ...
          break;
        case 'exponential':
          formula += '+(1|gr(cl)*fexp0(t))';
          break;
        case 'squared_exponential':
          formula += '+(1|gr(cl)*sqexp0(t))';
          break;
        default:
          formula += '+(1|gr(cl))';
      }
  ...
  _getWrapper(designId, design, options) {
    ...
    if (cached && cached.hash === hash) {
      // Reuse wrapper, just update parameters
      this._updateWrapperParameters(cached.wrapper, options, cached.meanN);
    ...
    // Clean up old wrapper if exists
    ...
    try {
      const success = wrapper.initialize(formula, wasmData, nRows, nCols, family, link);
      ...
      this._updateWrapperParameters(wrapper, options, meanN);
  ...
  // Update wrapper parameters without rebuilding model
  _updateWrapperParameters(wrapper, options, meanN) {
    const icc = options.icc ?? 0.05;
    const iac = options.iac ?? 0.0;
    const cacOrLengthscale = options.temporalCorrelation ?? options.cac ?? 0.8;
    const te = options.treatmentEffect ?? 0.5;
    const baseline = options.baseline ?? 0.0;
    
    // Set treatment effect separately (used by calculatePower)
    wrapper.setTreatmentEffect(te);
    wrapper.setAlpha(options.alpha ?? 0.05);
    wrapper.setTargetPower(options.targetPower ?? 0.80);
    wrapper.setIncludeIntercept(options.includeIntercept !== false);
    
    // Update covariance parameters
    wrapper.updateParameters(icc, iac, cacOrLengthscale, te, baseline, meanN);
  },
  ...
  calculateResultsWithClusters(design, options, totalClusters) {
    // For plotting - temporarily override cluster count
    ...
    // Scale clusters proportionally
    design._clustersPerSequence = originalClusters.map(c => 
      Math.max(1, Math.round(c * scale))
    );
    
    const result = this.calculateResults(design, options, '_temp_clusters');
...
const PlotArea = ({
  ...
  const currentXConfig = xAxisOptions.find((o) => o.value === xAxis);

  // Generate plot data
  const plotData = useMemo(() => {
    if (!currentXConfig) return [];
    ...
    return designs.map((d, idx) => {
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        if (xAxis === "totalClusters") {
          ...
          result = MathsInterface.calculateResults(d.design, modifiedOptions);
        }
  ...
  const surfaceData = useMemo(() => {
    ...
    // For surface plot, we use the first design only
    const d = designs[0];
    const clusterSizes = Array.from({ length: 25 }, (_, i) => 5 + i * 4); // 5 to 101
    const totalClusters = Array.from({ length: 25 }, (_, i) => 2 + i * 2); // 2 to 50
  ...
  }, [cacheVersion, plotType]); // Changed from [designs, plotType]
...
function App() {
  ...
  const loadPreset = (preset) => {
    updateActiveDesign((entry) => {
      ...
      switch (preset) {
        case "parallel":
          newDesign = TrialDesign.createParallel(4, 1);
          break;
        case "stepped-wedge":
        ...
        : "bg-white border-slate-300 text-slate-600 hover:bg-slate-50"
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-slate-700 text-white rounded-lg text-sm font-medium
